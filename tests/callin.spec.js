...
/* ── ใบที่ยกเลิกแล้ว ต้องไม่ถูกนับเป็น "เรารู้จัก Delta ไม่มี" ─────────────
 *
 * planCallIn() ใช้ activeOrders() อยู่แล้วตั้งแต่ #67 แต่ไม่มีเทสตรึง missingRow ไว้
 * CTO agent ย้อนให้ missingRow นับใบที่ยกเลิกด้วย แล้วเทส callin ทั้ง 4 ข้อยังเขียว
 *
 * ถ้าพัง แผงเทียบยอดจะฟ้องว่า "เรารู้จัก Delta ไม่มี" สำหรับใบที่เราปิดไปเองแล้ว
 * ซึ่งเป็นรายการที่เจ้าของเอาไปคุยกับ Delta — ฟ้องผิดบ่อย ๆ คนก็เลิกเชื่อทั้งหน้า */
test('ใบที่ยกเลิกแล้ว ต้องไม่ถูกนับว่า "เรารู้จัก Delta ไม่มี"', async ({ page }) => {
  const voided = Object.assign(order('PO-C040', '9100000040', 300), { voided: true });
  await openWith(page, [order('PO-C041', '9100000041', 300), voided]);

  // ไฟล์มีแต่ใบแรก · ใบที่สองเรายกเลิกไปแล้ว จึงไม่ควรถูกนับว่า "เราค้างแต่ Delta ไม่มี"
  await scan(page, await callInWorkbook([
    { pn: '9100000041', poNo: 'PO-C041', orderDate: '2026-07-06', qty: 300, wip: 180 }
  ]), 'X-FRM wk34');

  const sum = await page.locator('#callInSummary').innerText();
  expect(sum, 'ใบที่เรายกเลิกไปเองแล้ว ต้องไม่ถูกฟ้องว่า Delta ไม่มี')
    .toContain('ใบที่เรายังส่งไม่ครบ แต่ไม่มีแถวในไฟล์ 0');
  await expect(page.locator('#callInTable'), 'และต้องไม่โผล่ในตาราง')
    .not.toContainText('PO-C040');
});
