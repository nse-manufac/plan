...
/* ── issue #62 — ยอด Delta ของงวดเก่าต้องไม่โผล่มาแทนงวดที่ไม่มีข้อมูล ──────
 *
 * พนักงานแจ้ง 5 ก.ย. 2026 ว่าไฟล์ Call In ไม่มียอดของใบหนึ่ง แต่หน้าใบส่งของ
 * ยังโชว์ตัวเลข และตัวเลขนั้นบังเอิญเท่ากับ PO QTY พอดี จนคิดว่าโปรแกรมไปหยิบ PO QTY มา
 *
 * ของเดิม deltaWipOf() หยิบแถวที่ "เลขสัปดาห์สูงสุดเท่าที่เคยเก็บ" มาเสมอ ไม่มีวันหมดอายุ
 * ใบที่ตอนงวดเก่ายังไม่ได้ส่งอะไร ยอดค้างจึงเท่ากับยอดสั่งพอดี
 *
 * เลขนี้ถูกพิมพ์ลงช่อง Wip bal. ของ FM-ST-07 ที่ส่งให้ลูกค้าจริง ไม่ใช่แค่บนจอ */

/** แถวยอด Delta ที่คุมเวลาได้เอง — ใช้ทดสอบว่า "ล่าสุด" ตัดสินจากอะไร */
const dwAt = (orderId, week, wip, at) => ({
  id: 'DW-' + week + '-' + orderId, orderId, week: String(week), wip,
  fileName: 'callin-wk' + week + '.xlsx', deviceName: 't',
  createdAt: at, updatedAt: at, voided: false, _dirty: false
});

test('#62 — ใบที่ไม่มีในไฟล์งวดล่าสุด ต้องไม่โชว์เลขของงวดเก่า', async ({ page }) => {
  await openWithDelta(page, [
    // งวดเก่า wk34 เคยเก็บของ PO-B001 ไว้
    dwAt('PO-B001|' + PN_B, 34, 12000, '2026-08-20T00:00:00.000Z'),
    // งวดใหม่ wk36 มีแต่ PO-B055 — PO-B001 หายไปจากไฟล์
    dwAt('PO-B055|' + PN_B, 36, 500, '2026-09-05T00:00:00.000Z')
  ]);
  await page.uncheck('#dnHideNoDelta');
  await page.waitForTimeout(200);

  const cellOf = id => page.locator(`#dnTable td[data-deltawip="${id}"]`).innerText();
  expect(await cellOf('PO-B055|' + PN_B), 'ใบที่อยู่ในไฟล์ล่าสุด ต้องโชว์ยอดของงวดนั้น')
    .toContain('500');
  const gone = await cellOf('PO-B001|' + PN_B);
  expect(gone, 'ใบที่ไม่มีในไฟล์ล่าสุด ต้องขึ้นว่าไม่มียอด ไม่ใช่เลขของ wk34')
    .toContain('ไม่มียอด Delta');
  expect(gone, 'และต้องไม่มีเลขของงวดเก่าโผล่มา').not.toContain('12,000');
});

test('#62 — ประวัติของทุกงวดต้องยังอยู่ครบ ไม่ถูกลบหรือ void', async ({ page }) => {
  // เจ้าของเลือกไว้ว่า "เก็บประวัติไว้ แต่การแสดงผลถูกทับด้วยไฟล์ใหม่ตลอด"
  // การกรองจึงต้องอยู่ตอนแสดงผลเท่านั้น ห้ามไปแตะข้อมูลที่เก็บไว้
  await openWithDelta(page, [
    dwAt('PO-B001|' + PN_B, 34, 12000, '2026-08-20T00:00:00.000Z'),
    dwAt('PO-B055|' + PN_B, 36, 500, '2026-09-05T00:00:00.000Z')
  ]);
  ...
  expect(kept.deltaWip.length, 'ทุกแถวต้องยังอยู่').toBe(2);
  expect(kept.deltaWip.every(d => d.voided === false), 'ห้ามมีแถวไหนถูก void').toBe(true);
  expect(kept.deltaWip.find(d => d.week === '34').wip, 'ยอดของงวดเก่าต้องไม่ถูกแก้').toBe(12000);
});

test('#62 — งวดเก่าที่ถูกซิงค์ทับ updatedAt ให้ใหม่กว่า ต้องไม่ชนะงวดปัจจุบัน', async ({ page }) => {
  /* ⚠️ กรณีที่ผู้ตรวจจับได้ใน #63 — เกิดจริงและอันตรายที่สุดของใบนี้
   *
   *    หลัง push สำเร็จ ทุกแถวที่ _dirty ถูกตั้ง updatedAt = เวลาของเซิร์ฟเวอร์ (D4)
   *    และตอนบูต แถวที่ยังไม่เคยซิงค์ถูกตั้ง _dirty ทั้งหมด (D5)
   *
   *    เครื่องที่ออฟไลน์ค้างไว้พร้อมงวดเก่า พอกลับมาซิงค์จะ push งวดเก่าทีหลัง
   *    งวดเก่าจึงได้ updatedAt ใหม่สุด ถ้าตัดสินด้วย updatedAt ทั้งเครื่องจะย้อนไปงวดเก่า
   *    ใบของงวดปัจจุบันกลายเป็น "ไม่มียอด Delta" ทุกใบ แล้วถูกช่องติ๊กของ #61 ซ่อนหมด
   *    = ส่งของไม่ได้ทั้งหน่วย
   *
   *    createdAt เขียนครั้งเดียวตอนสร้าง (B4) การ push ไม่แตะ จึงนิ่งข้ามเครื่อง */
  await openWithDelta(page, [
    // งวดเก่า wk34 สร้างไว้นานแล้ว แต่เพิ่งถูกซิงค์ทับ updatedAt เมื่อวินาทีที่แล้ว
    Object.assign(dwAt('PO-B001|' + PN_B, 34, 12000, '2026-08-20T00:00:00.000Z'),
                  { updatedAt: '2026-09-30T23:59:59.000Z' }),
    // งวดปัจจุบัน wk36 สร้างทีหลัง แต่ updatedAt เก่ากว่า
    dwAt('PO-B055|' + PN_B, 36, 500, '2026-09-05T00:00:00.000Z')
  ]);
  await page.uncheck('#dnHideNoDelta');
  await page.waitForTimeout(200);

  expect(await page.locator(`#dnTable td[data-deltawip="PO-B055|${PN_B}"]`).innerText(),
    'งวดปัจจุบันต้องยังเป็นตัวที่ใช้').toContain('500');
  const old = await page.locator(`#dnTable td[data-deltawip="PO-B001|${PN_B}"]`).innerText();
  expect(old, 'งวดเก่าที่ถูกซิงค์ทับเวลา ต้องไม่กลายเป็นงวดล่าสุด').toContain('ไม่มียอด Delta');
  expect(old, 'และเลขของงวดเก่าต้องไม่โผล่มา').not.toContain('12,000');
});

test('#62 — ขึ้นปีใหม่ WK 1 ต้องชนะ WK 52 ของปีก่อน', async ({ page }) => {
  /* ⚠️ กับดักที่จะโผล่ตอนสิ้นปี ถ้าตัดสิน "งวดล่าสุด" จากเลขสัปดาห์
   *    ช่อง week เก็บแต่เลข ไม่มีปีกำกับ WK 1 จะแพ้ WK 52 ตลอดไป
   *    แล้วไฟล์ของปีใหม่จะไม่มีวันถูกใช้เลย ซึ่งแย่กว่าอาการที่แจ้งมาอีก */
  await openWithDelta(page, [
    dwAt('PO-B001|' + PN_B, 52, 9999, '2026-12-24T00:00:00.000Z'),
    dwAt('PO-B001|' + PN_B, 1, 111, '2027-01-07T00:00:00.000Z')
  ]);
  await page.uncheck('#dnHideNoDelta');
  await page.waitForTimeout(200);

  const cell = await page.locator(`#dnTable td[data-deltawip="PO-B001|${PN_B}"]`).innerText();
  expect(cell, 'ต้องใช้ยอดของ WK 1 ปีใหม่').toContain('111');
  expect(cell, 'ห้ามค้างอยู่ที่ WK 52 ของปีก่อน').not.toContain('9,999');
});
