...
/** แถวยอดค้างของ Delta หนึ่งแถว
 *
 *  ⚠️ createdAt ต้องต่างกันตามงวด ห้ามให้ทุกแถวเท่ากัน
 *
 *     ของเดิมตั้ง createdAt เป็นค่าเดียวกันหมด ทำให้เทส "มีหลายงวด ต้องใช้งวดที่ใหม่กว่า"
 *     ไปตกที่ตัวตัดสินสำรอง (เลขงวด) แทนที่จะเป็นกติกาหลักคือ createdAt
 *     ผลคือย้อน deltaWipOf ให้เลือกแถวเก่าสุด หรือย้อน currentDeltaWeek ให้ใช้เลขงวดสูงสุด
 *     แล้วเทสยังเขียวทั้งคู่ — คือไม่ได้คุมกติกาที่คอมเมนต์ ⚠️ ในโค้ดย้ำไว้สองที่เลย
 *
 *     ค่าเริ่มต้นให้งวดมากกว่า = นำเข้าทีหลัง ซึ่งเป็นลำดับปกติของงานจริง
 *     เคสข้ามปี (งวดน้อยกว่าแต่นำเข้าทีหลัง) ส่ง createdAt เองเป็นตัวที่สี่ */
...
/** เวลานำเข้าสมมติของงวดหนึ่ง — งวดมากกว่า = นำเข้าทีหลัง */
function weekStamp(week){
  const d = new Date(Date.UTC(2026, 0, 1) + Number(week) * 7 * 86400000);
...
/* ── ข้ามปี · งวดเลขน้อยกว่าแต่นำเข้าทีหลัง ต้องชนะ ──────────────────
 *
 * โค้ดเขียนคอมเมนต์ ⚠️ ย้ำไว้สองที่ (deltaWipOf กับ currentDeltaWeek) ว่า
 * "ตัดสินด้วย createdAt ห้ามเอาเลขงวดที่มากที่สุด · ปลายปี wk52 -> wk01
 *  เลขน้อยลงแต่ใหม่กว่า" — แต่ไม่มีเทสข้อไหนคุมเลยจนถึง 7 ก.ย. 2026
 *
 * ข้อนี้ข้อเดียวคุมทั้งสองฟังก์ชัน:
 *   ถ้า deltaWipOf เลือกแถวเก่าสุด    -> ได้แถว wk52 ซึ่งงวดไม่ตรงกับงวดล่าสุด -> แถวถูกซ่อน
 *   ถ้า currentDeltaWeek ใช้เลขงวดสูงสุด -> ได้ '52' ซึ่งไม่ตรงกับแถว wk1 -> แถวถูกซ่อนเช่นกัน
 * ทั้งสองทางทำให้ใบหายจากจอ ซึ่งแปลว่าส่งของใบนั้นไม่ได้ */
test('ข้ามปี — งวด wk1 ที่นำเข้าทีหลัง ต้องชนะ wk52 ที่นำเข้าก่อน', async ({ page }) => {
  await openWithDelta(page, [
    deltaRow('PO-B001|' + PN_B, 52, 9000, '2026-12-28T00:00:00.000Z'),
    deltaRow('PO-B001|' + PN_B, 1, 4200, '2027-01-04T00:00:00.000Z')
  ]);

  const row = page.locator('#dnTable tbody tr').filter({ hasText: 'PO-B001' });
  await expect(row, 'ใบต้องยังอยู่บนจอ ไม่ใช่ถูกซ่อนเพราะงวดไม่ตรงกัน').toHaveCount(1);
  await expect(row, 'ต้องใช้ยอดของงวดที่นำเข้าทีหลัง').toContainText('4,200');
  await expect(row, 'และบอกว่าเป็นงวด wk1').toContainText('wk1');
  await expect(page.locator('#dnSummary'), 'ยอดสรุปต้องยึดงวด wk1 ด้วย')
    .toContainText('ยึดยอด Delta จากไฟล์งวด wk1');
});
...
/* ── ยอดที่หักแล้ว ใช้เฉพาะหน้ายอดคงคลัง FG ──────────────────────────────
 *
 * เจ้าของเลือกให้หน้า FG หักยอดที่เราส่งแล้วแต่ Delta ยังไม่นับ (14–15 ก.ย. 2026)
 * หน้าใบส่งสินค้าและกระดาษยังเป็นยอดของ Delta ตรง ๆ · ข้อนี้คุมไม่ให้ใครเผลอเอาตัวหักมาใช้ที่นี่
 * ฝั่งที่หักอยู่ใน fg.spec.js
 *
 * ⚠️ ข้อมูลต้องเป็นกรณีที่หน้า FG "หักจริง" (Wip bal. เท่ายอดสั่ง = Delta ยังไม่นับยอดส่งเลย)
 *    ไม่งั้นตัวหักคืนค่าเท่าเดิม แล้วเทสเขียวเสมอแม้หน้านี้จะเผลอใช้ตัวหัก */
const shipRec = (id, orderId, date, qty) => ({
...
test('หน้าใบส่งสินค้าและกระดาษใช้ Wip bal. ของ Delta ตรง ๆ — ไม่หักยอดที่ Delta ยังไม่นับ', async ({ page }) => {
  const ID = 'PO-B001|' + PN_B;                 // ยอดสั่ง 12,000
  await openWithDelta(page, [deltaRow(ID, 35, 12000, '2026-09-01T05:00:00.000Z')], ORDERS, [
    shipRec('s2', ID, '2026-09-03', 1200)     // Delta ยังไม่นับ — หน้า FG หัก แต่หน้านี้ห้ามหัก
  ]);
  ...
  const cell = await page.locator(`#dnTable td[data-deltawip="${ID}"]`).innerText();
  expect(cell, 'บนจอต้องเป็นยอดในไฟล์ของ Delta').toContain('12,000');
  expect(cell, 'ห้ามมีบรรทัดบอกยอดที่หัก').not.toContain('ยังไม่นับ');
